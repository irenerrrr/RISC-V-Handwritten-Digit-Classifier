...
class TestAbsLoss(unittest.TestCase):
    def test_simple(self):
        ...
        array0 = t.array([-1, 0 , 1, 4])
        # load address of `array0` into register a0
        t.input_array("a0", array0)
        # create array1 in the data section
        array1 = t.array([1, 0, -2, -8])
        ...
        t.call("abs_loss")
        t.check_array(array2, [2,0,3,12])
        # check that the register a0 contains the correct output
        t.check_scalar("a0", 17)
        # generate the `assembly/TestAbsLoss_test_simple.s` file and run it through venus
        t.execute()
    # Add other test cases if neccesar
    def test_inputbadTest(self):
        t = AssemblyTest(self, "../coverage-src/abs_loss.s")
        t.input_scalar("a2", -1)
        t.call("abs_loss")
        t.execute(code=36)
...
class TestSquaredLoss(unittest.TestCase):
    def test_simple(self):
        ...
        # call the `abs_loss` function
        t.call("squared_loss")
        t.check_array(array2, [4,0,9,144])
        # check that the register a0 contains the correct output
        t.check_scalar("a0", 157)
        # generate the `assembly/TestAbsLoss_test_simple.s` file and run it through venus
        t.execute()
...
class TestInitializeZero(unittest.TestCase):
    ...
    # Add other test cases if neccesary
    def test_inputbadTest36(self):
        t = AssemblyTest(self, "../coverage-src/initialize_zero.s")
        t.input_scalar("a0", -1)
        t.call("initialize_zero")
        t.execute(ßode=36)
    def test_intputBadTest26(self):
        t = AssemblyTest(self, "../coverage-src/initialize_zero.s")
        t.input_scalar("a0", 123456788)
        t.call("initialize_zero")
